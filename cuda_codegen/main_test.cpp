...
int main(void){
    ...
    {
        CUDA_SAFE_CALL(cudaMemcpy(Parameter_270_0, Parameter_270_0_host, sizeof(float) * 150528, cudaMemcpyHostToDevice));
        kernel_entry(Parameter_270_0, &Result_505_0);
        CUDA_SAFE_CALL(cudaMemcpy(Result_505_0_host, Result_505_0,  sizeof(float) * 1001, cudaMemcpyDeviceToHost));
        CUDA_SAFE_CALL(cudaDeviceSynchronize()); 
        printf("%s \n", "Result_505_0:");
        for (int i = 0; i < 10; ++i) printf("%e ", (float)Result_505_0_host[i]); 
        printf(" .. (size = 1001, ends with %e);\n", (float)Result_505_0_host[1000]);
    }
    ...
    //free context
    CUDA_SAFE_CALL(cudaFree(Parameter_270_0));
    cuda_free();

    cudaFreeHost(Parameter_270_0_host);
