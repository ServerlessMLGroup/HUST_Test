...
def main():
    ...
    for i in range(1, 3):
        ...
        os.environ['CUDA_MPS_ACTIVE_THREAD_PERCENTAGE'] = "22"
        worker = WorkerProc(("worker-22-%d" % i), p_child_worker, mps_percentage=22, batch_size=16, nruns=300)
        worker.start()
    ...
    for i in range(1, 3):
        p_parent_worker, p_child_worker = mp.Pipe()
        os.environ['CUDA_MPS_ACTIVE_THREAD_PERCENTAGE'] = "33"
        worker = WorkerProc(("worker-33-%d" % i), p_child_worker, mps_percentage=33, batch_size=32, nruns=300)
        worker.start()
        worker_meg_list.append(p_parent_worker)
        worker_list.append(worker)
    ...
    # for i in range(1, 3):
    #     p_parent_worker, p_child_worker = mp.Pipe()
    #     os.environ['CUDA_MPS_ACTIVE_THREAD_PERCENTAGE'] = "35"
    #     worker = WorkerProc(("worker-35-%d" % i), p_child_worker, mps_percentage=35, batch_size=32, nruns=300)
    #     worker.start()
    #     worker_meg_list.append(p_parent_worker)
    #     worker_list.append(worker)
    ...
    #     worker = WorkerProc(("worker-35-%d" % i), p_child_worker, mps_percentage=10, batch_size=32, nruns=300)
    #     worker.start()
    #     worker_meg_list.append(p_parent_worker)
    #     worker_list.append(worker)
